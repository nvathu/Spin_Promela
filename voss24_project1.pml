...
proctype Tower() {
	...
	:: plane_state?AIR_GO_AROUND(i) -> // plane in the air ready for landing
		atomic {
			if 
			:: !runway && !taxiway && gates_count < 4 -> runway = true;tower_cmd!RUNWAY_LAND(i);// prevent from landing if plane will be stuck
			:: else -> printf("Airplane %d goes around \n",i);plane_state!AIR_GO_AROUND(i);
			fi;
		}
	:: plane_state?RUNWAY_LAND(i) -> // plane just landed
		atomic {
			if
			:: !taxiway && gates_count < 4 -> taxiway = true;tower_cmd!TAXIWAY_LAND(i);runway = false;// go to the taxiway if free and there is space in gates
			:: else -> printf("Airplane %d is waiting on the runway \n",i);plane_state!RUNWAY_LAND(i);
			fi;
		}
	:: plane_state?TAXIWAY_LAND(i) -> // plane on the taxiway,want to go to gate
		atomic {
			if
			:: gates_count < 4 -> gates_count = gates_count + 1;tower_cmd!GATE(i);taxiway = false;// go if some gate is free
			:: else -> printf("Airplane %d is waiting on the taxiway \n",i);plane_state!TAXIWAY_LAND(i);
			fi;
		}
	:: plane_state?GATE(i) -> // plane stands in the gate
		atomic {
			if
			:: !taxiway && !runway -> taxiway = true;tower_cmd!TAXIWAY_TAKE_OFF(i);gates_count = gates_count - 1;// go to the taxiway if it is free and runway is free
			:: else -> printf("Airplane %d is waiting in the gate \n",i);plane_state!GATE(i);
			fi;
		}
	:: plane_state?TAXIWAY_TAKE_OFF(i) -> // plane on the taxiway,want to take off
		atomic {
			if
			:: !runway -> runway = true;tower_cmd!RUNWAY_TAKE_OFF(i);taxiway = false;// go on the runway to take off if free
			:: else -> printf("Airplane %d is wainting on the taxiway \n",i);plane_state!TAXIWAY_TAKE_OFF(i);
			fi;
		}
...
proctype Airplane(int id) {
	...
	:: tower_cmd?GATE(id) -> 
		...
		plane_state!GATE(id);
		goto Gate;
	:: tower_cmd?RUNWAY_TAKE_OFF(id) -> printf("Airplane %d going to takeoff\n",id);plane_state!RUNWAY_TAKE_OFF(id);goto Runway;
	fi;
	
	Gate:
	if
	:: tower_cmd?TAXIWAY_TAKE_OFF(id) -> printf("Airplane %d taxing to take off\n",id);plane_state!TAXIWAY_TAKE_OFF(id);goto Taxiway;
	fi;
}
